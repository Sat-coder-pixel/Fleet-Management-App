    ...
    return (
      <Animated.View style={[styles.card, { transform: [{ translateY }], opacity }]}>
        <View style={styles.cardContent}>
          ...
          <ThemedText style={[styles.status, { marginTop: 8 }]}>{item.status}</ThemedText>
...
const styles = StyleSheet.create({
  ...
  desc: { color: '#444', marginTop: 6 },
  small: { color: '#666', marginTop: 6 },
  action: { marginLeft: 12, alignItems: 'flex-end' },
  status: { fontWeight: '600', marginBottom: 6, color: '#1b7ed6' },
});
...
//       Alert.alert('Success', 'Task started');
//       // update local state
//       setTasks((t) => t.map((item) => (item.assignedTaskId === assignedTaskId ? { ...item, status: 'In Progress' } : item)));
//       await storage.saveTasksForTruck(driver.truckNo, tasks);
//     } catch (e: any) {
//       Alert.alert('Error', String(e));
//     }
//   }
...
//   return (
//     <SafeAreaView style={styles.container}>
//       <ThemedText type="title">Tasks for Truck #{driver?.truckNo}</ThemedText>
//       <ThemedText>{driver?.driverName}</ThemedText>
...
//             <ThemedText>Qty: {item.quantityShipped ?? item.quantityOrdered ?? '-'}</ThemedText>
//             <ThemedText>Status: {item.status}</ThemedText>
//             <View style={{ marginTop: 8 }}>
//               <Button title="Start task" onPress={() => onStartTask(item.assignedTaskId)} />
//             </View>
//           </View>
//         )}
//       />
